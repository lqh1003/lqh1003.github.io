...
- 修改「全局」user.name/user.email（所有仓库通用）
  ...
  ```
- 验证全局配置（点击可复制）
  - 账号1：<a href="javascript:;" onclick="navigator.clipboard.writeText('git config --global user.name liqinghua-ty')">liqinghua-ty</a> / <a href="javascript:;" onclick="navigator.clipboard.writeText('git config --global user.email liqinghua@tanyue.cc')">liqinghua@tanyue.cc</a>
  - 账号2：<a href="javascript:;" onclick="navigator.clipboard.writeText('git config --global user.name lqh1003')">lqh1003</a> / <a href="javascript:;" onclick="navigator.clipboard.writeText('git config --global user.email 2933213867@qq.com')">2933213867@qq.com</a>
  ```bash
